...
`SECRET_KEY` y Docker Compose usa sus valores. El `.env` está ignorado por Git.
...
Copiá la clave generada después de `SECRET_KEY=` y ejecutá:
...
2. Construye las imágenes de nivel 1 y nivel 2.
3. Composer instala `firebase/php-jwt` dentro de cada imagen.
4. Crea una red privada para los tres servicios.
5. Crea el volumen `mysql_data`.
6. MySQL ejecuta `nivel-2/database.sql` y carga los datos iniciales.
7. Cuando MySQL está saludable, arrancan las dos APIs.
...
Cambiá `NIVEL_1_PORT`, `NIVEL_2_PORT` o `MYSQL_PORT` en el `.env` raíz.
...
- Los puertos 8001 y 8002 permiten entrar desde la computadora.
